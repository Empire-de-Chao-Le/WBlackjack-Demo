...
/**
 * GET /flashcards/due/:language?today=YYYY-MM-DD&limit=20
 * Returns the ordered session card ids (due cards first, then new/unseen cards),
 * plus availability counts. Cards reviewed today are never due again today, so
 * the same question cannot appear twice within one day.
 */
router.get("/flashcards/due/:language", async (req, res): Promise<void> => {
  ...
  const limit = Math.max(1, Math.min(100, parseInt(String(req.query.limit ?? SESSION_LIMIT)) || SESSION_LIMIT));
  ...
  // Session: all due cards (capped at limit), then fill with new cards.
  const session: number[] = shuffle(dueIds).slice(0, limit);
  if (session.length < limit) {
    const room = Math.min(limit - session.length, MAX_NEW_PER_SESSION);
    session.push(...shuffle(newIds).slice(0, room));
  }

  // `session` is already due-first then new (each bucket shuffled internally);
  // keep that order so due cards are always studied before new ones.
  res.json({
    cardIds: session,
